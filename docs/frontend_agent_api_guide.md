...
- Public read endpoints exist for item categories, user items, user item images, collections, collection items, branch items, donor groups, donor group members, donor group items, and courier profiles.
- Authenticated users create their own `user-items` and `courier-profiles`.
- Authenticated users can upload optional photos for their own user items through `user-item-images`.
...
- Collection authors and organization managers complete donor group delivery with `POST /donor-groups/{id}/complete-delivery/`; after that donor group items, chat writes, and media writes are archived.
- Completed donor groups can be hidden from default frontend lists with `POST /donor-groups/{id}/hide/`; use `include_hidden=true` to list them.
- Donor group members can upload video reports attached directly to the donor group before delivery is completed.
...
The `collections` app has a public REST API now. Organization and donor group chats are implemented in `communications`. Poll-result finalization for donor group date/place parameters, donor group delivery completion, delivered item summaries, user item photos, and donor group video reports are implemented.
...
- Treat `DonorGroupItem` as editable planned quantity while `DonorGroup.status = active`, and as delivered quantity after `delivery_completed`.
...
GET    /api/v1/collections/delivered-items/
GET    /api/v1/collections/delivered-items/?organization=1
GET    /api/v1/collections/delivered-items/?collection=1
GET    /api/v1/collections/delivered-items/?branch=1
GET    /api/v1/collections/delivered-items/?user=1
GET    /api/v1/collections/delivered-items/summary-by-category/
GET    /api/v1/collections/delivered-items/summary-by-user/
```
...
- User item images: public read; authenticated item owners upload, update, or delete photos for their own items.
...
- Donor groups: public read; the collection author or an active organization manager creates, updates, deletes active groups, completes delivery, hides completed groups, or unhides groups.
...
- Donor group items: public read; while the group is active, the user item owner, collection author, or active organization manager can manage selected quantities. After delivery completion, nobody can change quantities.
- Delivered items: authenticated read; users see their own delivered items, collection authors see their collection deliveries, and organization managers see deliveries for their organizations. Supports `organization`, `collection`, `branch`, `donor_group`, `user`, and `category` filters.
- Donor group video reports: authenticated read for donor group members, the collection author, and active organization managers; donor group members upload before completion; uploader or group manager edits/deletes before completion.
...
Complete donor group delivery:

```http
POST /api/v1/collections/donor-groups/{id}/complete-delivery/
```

This sets `status = "delivery_completed"`, writes `completed_at` / `completed_by_member`, archives the donor group chat and media writes, and freezes donor group item quantities.

Hide completed donor group from default frontend lists:

```http
POST /api/v1/collections/donor-groups/{id}/hide/
POST /api/v1/collections/donor-groups/{id}/unhide/
```

Default donor group list excludes hidden groups. Pass `include_hidden=true` to include them.

Donor group responses include `status`, `is_hidden`, completion metadata, `video_reports_count`, and `delivered_quantity_total`.
